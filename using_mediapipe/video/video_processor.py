...
IMAGE_SAVE_PATH = os.path.join("..", "..", "data",  "saved_images")
MIN_DETECTION_CONFIDENCE = 0.75
SAVE_EMBEDDING_RATE = 10


def save_encodings_to_file(embeddings_file_name, file_location):
    sfc = SimpleFacerec(embeddings_file_name)
...
def save_random_image(frame, embeddings, frame_duration_counter):
    for nr in range(len(embeddings)):
        emb = embeddings[nr]
        if random.randint(1, 100) > SAVE_EMBEDDING_RATE:
            continue
...
def take_pictures(video_file_name):
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            try:
                ...
                save_random_image(frame, embeddings, frame_duration_counter)
            except Exception as e2:
                print(e2)
        else:
            break
...
def run_face_detector(video_file_name, embeddings_file_name):
    sfc = SimpleFacerec(embeddings_file_name)
